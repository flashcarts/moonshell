...
#ifdef PNG_DEBUG
#define png_debug(l,m) _consolePrintf(m)
...
#include "libpng/png.h"
  ...
  typedef u32                * png_FILE_p;
//#define stdout (NULL)
#endif

int FileHandle;

png_FILE_p _fopen(int _FileHandle)
{
  FileHandle=_FileHandle;
  
  return((png_FILE_p)FileHandle);
}

void _fclose(png_FILE_p fp)
{
  FileHandle=0;
}

size_t _fread(void *buf, size_t size, size_t n, png_FILE_p fp)
{
  return((size_t)fread(buf,size,n,(int)fp));
}

size_t _fwrite(const void *buf, size_t size, size_t n, png_FILE_p fp)
{
  return(0);
}
...
png_debug_malloc(png_structp png_ptr, png_uint_32 size)
{
   ...
   if (png_ptr == NULL){
     _consolePrintf("error: malloc png_ptr=NULL\n");
     while(1);
     return(NULL);
   }
   if (size==0){
     _consolePrintf("error: malloc size=0\n");
     while(1);
     return(NULL);
   }
      ...
      buffer and once to get a new free list entry. */
   {
      ...
      pinfo = (memory_infop)safemalloc((png_uint_32)png_sizeof (*pinfo));
      pinfo->size = size;
      ...
      pinfo->pointer = (png_voidp)safemalloc(size);
         ...
         if (pinfo->pointer == ptr)
         {
            *ppinfo = pinfo->next;
            ...
            if (current_allocation < 0)
               _consolePrintf("Duplicate free of memory\n");
...
png_voidp ndspng_malloc(png_structp png_ptr, png_uint_32 size)
{
   ...
   if (png_ptr == NULL){
     _consolePrintf("error: ndspng_malloc png_ptr=NULL\n");
     while(1);
     return(NULL);
   }
   if (size==0){
     _consolePrintf("error: ndspng_malloc size=0\n");
     while(1);
     return(NULL);
   }
  ...
  ret=(png_voidp)safemalloc(size);
  
  if(ret==NULL){
    _consolePrintf("error: ndspng_malloc out of memory.");
    while(1);
    return(NULL);
  }
...
{
   if (png_ptr == NULL){
     _consolePrintf("error: freeing NULL pointer\n");
     while(1);
     return;
   }
   if (ptr == 0)
...
static b8 __Start(int FileHandle)
{
   ...
   {
      _consolePrintf("Could not find input buffer.\n");
      return(False);
   }
...
//   if (color_type == PNG_COLOR_TYPE_RGB || color_type == PNG_COLOR_TYPE_RGB_ALPHA) png_set_bgr(read_ptr);
   ...
   row_buf = (png_bytep)safemalloc(DecodeBufSize+16); // 16byte dummy
   if(row_buf==NULL){
     _consolePrintf("row_buf at NULL\n");
     while(1);
     return(False);
   }
