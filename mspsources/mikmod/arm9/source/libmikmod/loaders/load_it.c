...
BOOL IT_Load(BOOL curious)
{
	...
	MWin_ProgressShow("LoadSamples",mh->smpnum);
	
	/* Load all samples */
	q = of.samples;
	for(t=0;t<mh->smpnum;t++) {
		MWin_ProgressSetPos(t);
	...
	if(mh->flags&4) {
		...
		MWin_ProgressShow("LoadInstruments",mh->insnum);
...
#undef IT_ProcessEnvelope
	...
	MWin_ProgressShow("LoadPattern",of.numpat);
	...
	MWin_ProgressShow("ConvertPattern",of.numpat);
