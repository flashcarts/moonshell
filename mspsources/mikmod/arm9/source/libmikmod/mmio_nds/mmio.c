...
#define fopen MikMod_fopen
#define fclose MikMod_fclose
#define fgetc MikMod_fgetc
#define fread MikMod_fread
#define fseek MikMod_fseek
#undef feof
#define feof MikMod_feof
#define ftell MikMod_ftell

extern FILE* MikMod_fopen(CHAR* fname,CHAR* attrib);
extern int MikMod_fclose(FILE *);
extern int MikMod_fgetc(FILE *);
//extern int MikMod_fputc(int, FILE *);
extern size_t MikMod_fread(void *, size_t, size_t, FILE *);
extern int MikMod_fseek(FILE *, long, int);
//extern size_t MikMod_fwrite(const void *, size_t, size_t, FILE *);
extern int MikMod_feof(FILE *fp);
extern long MikMod_ftell(FILE *fp);
...
{
	UWORD result=0;
	_mm_FileReader_Read(reader,&result,2);
	return(result);
...
{
	ULONG result=0;
	_mm_FileReader_Read(reader,&result,4);
	return(result);
