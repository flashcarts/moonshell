...
{
//_consolePrintf("SL_LoadInt inf=0x%x,outf=0x%x,sf=%d\n",infmt,outfmt,scalefactor);
	...
	status.bits=0;    /* current number of bits */
	status.bufbits=0; /* bits in buffer */
	status.last=0;    /* last output */
	status.buf=0;     /* bit buffer */
...
static BOOL DitherSamples(SAMPLOAD* samplist,int type)
{
	...
	MWin_ProgressShow("DitherSamples1",ListCount);
	...
	MWin_ProgressShow("DitherSamples2",ListCount);
