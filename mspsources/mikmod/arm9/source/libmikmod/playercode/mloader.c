...
MODULE* Player_LoadGeneric_internal(MREADER *reader,int maxchan,BOOL curious)
{
	...
	_consolePrintf("SL_LoadSamples();\n");
	if(SL_LoadSamples()) {
		_mm_iobase_revert();
		Player_Free_internal(mf);
		return NULL;
	}
	_consolePrintf("Player_Init(mf);\n");
	if(Player_Init(mf)) {
		_mm_iobase_revert();
		Player_Free_internal(mf);
		mf=NULL;
	...
	_consolePrintf("Player_LoadGeneric_internal done.\n");
	return mf;
}
