...
function bmp2b15(var pBMPFile:TBMPFile):boolean;
...
procedure saveb15(var pBMPFile:TBMPFile;w,h:integer);
var
  idx:integer;
  procedure write16(d:word);
  begin
    pBMPFile.DecompData[pBMPFile.DecompSize+0]:=(d shr 0) and $ff;
    pBMPFile.DecompData[pBMPFile.DecompSize+1]:=(d shr 8) and $ff;
    inc(pBMPFile.DecompSize,2);
  end;
begin
  setlength(pBMPFile.DecompData,B15DataCount*2);

  for idx:=0 to B15DataCount-1 do begin
    write16(B15Data[idx]);
  end;
  
  setlength(pBMPFile.DecompData,pBMPFile.DecompSize);
end;
...
begin
  ...
  dither(srcbm);
  saveb15(pBMPFile,srcbm.Width,srcbm.Height);
