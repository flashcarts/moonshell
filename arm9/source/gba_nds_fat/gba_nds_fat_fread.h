...
void FAT_Disabled64kClusterMode(void)
{
...
u32 FAT_fread (void* buffer, u32 size, u32 count, FAT_FILE* file)
{
  ...
  if ((file == NULL) || (file->inUse == false)){
    _consolePrintf("Fatal error: Can not read. not opened.\n");
    ShowLogHalt();
    return 0;
  }
  if (length == 0 || buffer == NULL){
    _consolePrintf("Fatal error: Can not read. Illigal size or buffer. (%d,%x)\n",length,buffer);
    ShowLogHalt();
    return 0;
  }
  ...
  if (!file->read){
    _consolePrintf("Fatal error: Can not read. for not read.\n");
    ShowLogHalt();
    return 0;
  }
