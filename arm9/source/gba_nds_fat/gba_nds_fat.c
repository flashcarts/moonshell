...
FAT_FILE* FAT_fopen(const char* path, const char* mode)
{
  ...
  if (pchTemp == NULL)
  {
    _consolePrintf("Fatal error: File open mode error.\n");
    ShowLogHalt();
    return NULL;
  }
  ...
  {
    _consolePrintf("Fatal error: Can not open directory.\n");
    ShowLogHalt();
    return NULL;
  }
  ...
  {
    _consolePrintf("Fatal error: Can not open ReadOnly file for write.\n");
    ShowLogHalt();
    return NULL;
  }
#else
  ...
  {
    _consolePrintf("Fatal error: Not impliments file write.\n");
    ShowLogHalt();
    return NULL;
  }
#endif
  ...
  {
    _consolePrintf("Fatal error: FAT_FILE buffer full.\n");
    ShowLogHalt();
    return NULL;
  }
  ...
  if ( strpbrk(mode, "wW") != NULL ) // (ucase(mode[0]) == 'W')
  {
    ...
    if (dirEntry.name[0] == FILE_FREE)  // No file
    {
      ...
      {
         _consolePrintf("Fatal error: Can not add dir entry.\n");
        ShowLogHalt();
        return NULL;
      }
  ...
  _consolePrintf("Fatal error: Unknown error in file open.\n");
  ShowLogHalt();
  return NULL;
}
...
bool FAT_fclose (FAT_FILE* file)
{
  ...
  if ((file != NULL) && (file->inUse == true))
  {
    ...
    {
      if(file->length==0){
        _consolePrintf("Fatal error: FileSystem: Not support 0byte file writed.\n");
        ShowLogHalt();
      }
...
u32 FAT_ftell (FAT_FILE* file)
{
  ...
  else
  {
    ...
    _consolePrintf("Fatal error: Can not tell. not opened.\n");
    ShowLogHalt();
    return -1;
  }
}
...
int FAT_fseek(FAT_FILE* file, s32 offset, int origin)
{
  ...
  {
    _consolePrintf("Fatal error: Can not seek. not opened.\n");
    ShowLogHalt();
    return -1;
  }
  ...
  {
    _consolePrintf("Fatal error: Can not seek in modify mode.\n");
    ShowLogHalt();
    return -1;
  }
  ...
  case SEEK_END:
    ...
    break;
  default:
    _consolePrintf("Fatal error: Can not seek. unknown origin mode.\n");
    ShowLogHalt();
    return -1;
  }
...
u32 FAT_fwrite (const void* buffer, u32 size, u32 count, FAT_FILE* file)
{
  ...
  if ((file == NULL) || (file->inUse == false)){
    _consolePrintf("Fatal error: Can not write. not opened.\n");
    ShowLogHalt();
    return 0;
  }
  ...
  {
    _consolePrintf("Fatal error: Can not write. opened for read.\n");
    ShowLogHalt();
    return 0;
  }
...
int FAT_remove (const char* path)
{
  ...
  if (dirEntry.name[0] == FILE_FREE)
  {
    _consolePrintf("Fatal error: Can not remove. This is removed.\n");
    ShowLogHalt();
    return -1;
  }
  ...
  if (dirEntry.attrib & ATTRIB_DIR)
  {
    ...
    if (checkFiletype != FT_NONE)
    {
      ...
      _consolePrintf("Fatal error: Can not tell. Directory isn't empty.\n");
      ShowLogHalt();
      return -1;
    }
  }
...
int FAT_mkdir (const char* path)
{
  ...
  {
    _consolePrintf("Fatal error: Can not mkdir. Directory is exists.\n");
    ShowLogHalt();
  ...
  while (path[pathPos + filePos] != '\0')
  {
    if (path[pathPos + filePos] == '/')
    {
      ...
      if (FAT_chdir(pathname) == false) 
      {
        ...
        _consolePrintf("Fatal error: Can not mkdir. Can not create directory.\n");
        ShowLogHalt();
  ...
  {
    _consolePrintf("Fatal error: Can not tell. Couldn't get a new cluster for the directory.\n");
    ShowLogHalt();
  ...
  {
    _consolePrintf("Fatal error: Can not tell. Couldn't add the directory entry.\n");
    ShowLogHalt();
