...
static CODE_IN_ITCM int get_motion_delta (mpeg2_decoder_t * const decoder,
                  const int f_code)
{
#define bit_buf (decoder->bitstream_buf)
#define bits (decoder->bitstream_bits)
...
static CODE_IN_ITCM void get_mpeg1_intra_block (mpeg2_decoder_t * const decoder)
{
  int i;
  int j;
  int val;
...
static CODE_IN_ITCM void motion_mp1 (mpeg2_decoder_t * const decoder,
...
static CODE_IN_ITCM void motion_reuse_##FORMAT (mpeg2_decoder_t * const decoder,        \
                   motion_t * const motion,            \
                   mpeg2_mc_fct * const * const table)        \
{\
  int motion_x, motion_y;                            \
...
static CODE_IN_ITCM void motion_zero_##FORMAT (mpeg2_decoder_t * const decoder,        \
                  motion_t * const motion,            \
                  mpeg2_mc_fct * const * const table)        \
{\
  unsigned int offset;                            \
...
void CODE_IN_ITCM mpeg2_slice (mpeg2_decoder_t * const decoder, const int code, const uint8_t * const buffer)
{
#define bit_buf (decoder->bitstream_buf)
#define bits (decoder->bitstream_bits)
#define bit_ptr (decoder->bitstream_ptr)
  ...
  while (1) {
    ...
    if (macroblock_modes & MACROBLOCK_QUANT) {
      _consolePrintf("Fatal error: macroblock_modes & MACROBLOCK_QUANT not implimented.\n");
      while(1);
//      get_quantizer_scale (decoder);
    }
